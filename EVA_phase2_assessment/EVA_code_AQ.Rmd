...
my_packages <- c( "data.table","sf", "raster", "raster",   
...
```{r setting2}
CreateGrid<-TRUE
GridSize<-250 #3km=3000 or 250m=250
```
...
# Benthos ESS
#ESS <- c("Abra alba", "Asterias rubens", "Bathyporeia pilosa", "Bathyporeia sarsi", "Eumida sanguinea", "Eurydice affinis", "Eurydice pulchra", "Kurtiella bidentata", "Nephtys hombergii", "Pariambus typicus", "Pygospio elegans", "Scolelepis squamata", "Fabulina fabula", "Pomatoschistus lozanoi", "Pomatoschistus microps", "Pomatoschistus minutus")
...
produce_map_sf <- function(dataset,class_column,name_AQ){
  ...
  print(ggplot()+
     ...
     geom_sf(data = add_grid_geometry, aes(fill=as.factor(.data[[name_AQ]])), colour = "grey90", lwd = 0) + 
     scale_fill_manual(values = viridisLite::viridis(5), name = name_AQ, limits = factor(c(1,2,3,4,5)), na.value = "grey94", labels = c("1" = "Very Low",
     "2" = "Low",
     "3" = "Medium",
     "4" = "High",
     "5" = "Very High"
...
produce_map(EcoComp1.3,"all_dataset", "Point of all the data available")
...
section (7.1 to 7.17), even if no ecologically significant
...
# Classification of the results into five categories
...
### 7.1 Create a dataset for qualitative and quantitative data

```{r dfqual_quant}

# Qualitative AQs can be answered using all available data
Qualitative_data <- EcoComp1.3
...
### 7.2 Get a list of rare and common species

#### 7.2.1 Rare and common species list per subzone.

```{r raresp}
# Creating a list of unique grid_id -  combinations
RareSpecList1<-aggregate(ID~ grid_id+ScientificName, data = Qualitative_data, length) 
...
Rarespec_loc_qualitative <- Qualitative_data[Qualitative_data$ScientificName %in% RareSpecList$ScientificName,]
st_write(Rarespec_loc_qualitative, file.path(Output,"locally_rare_species_occurence_distribution.shp"), append = FALSE)
produce_map(Rarespec_loc_qualitative,"locally_rare_sp_occurence", "Locally rare species occurence data available")

Rarespec_loc_quantitative <- Quantitative_data[Quantitative_data$ScientificName %in% RareSpecList$ScientificName,]
st_write(Rarespec_loc_qualitative, file.path(Output,"locally_rare_species_abundance_distribution.shp"), append = FALSE)
...
Rarespec_loc_qualitative_count <- Rarespec_loc_qualitative %>%
...
print(kbl(list(Rarespec_loc_qualitative_count,Rarespec_loc_quantitative_count))%>%
  kable_classic(full_width = F, html_font = "Cambria"))

write.csv(Rarespec_loc_qualitative_count, file = file.path(Output, paste0(OutputFileName,"_local_rare_sp_occurence_count.csv")), row.names = FALSE)
...
#### 7.2.2 Rare species list on Regional Scale
...
Rarespec_reg_qualitative <- Qualitative_data[Qualitative_data$ScientificName %in% RareSpecList_region$ScientificName,]
st_write(Rarespec_reg_qualitative, file.path(Output,"regionally_rare_species_occurence_distribution.shp"), append = FALSE)
produce_map(Rarespec_reg_qualitative,"regionally_rare_sp_occurence", "Regionally rare species occurence data available")
...
Rarespec_reg_qualitative_count <- Rarespec_reg_qualitative %>%
  group_by(ScientificName) %>% 
  summarise(occurence_data_present = n(), .groups="drop") %>%
  st_drop_geometry()
...
print(kbl(list(Rarespec_reg_qualitative_count,Rarespec_reg_quantitative_count))%>%
  kable_classic(full_width = F, html_font = "Cambria"))

write.csv(Rarespec_reg_qualitative_count, file = file.path(Output, paste0(OutputFileName,"_region_rare_sp_occurence_count.csv")), row.names = FALSE)
...
#### 7.2.3 Rare species list on National Scale
...
Rarespec_nat_qualitative <- Qualitative_data[Qualitative_data$ScientificName %in% RareSpecList_nation$ScientificName,]
st_write(Rarespec_nat_qualitative, file.path(Output,"nationally_rare_species_occurence_distribution.shp"), append = FALSE)
produce_map(Rarespec_nat_qualitative,"nationally_rare_sp_occurence", "Nationally rare species occurence data available")
...
Rarespec_nat_qualitative_count <- Rarespec_nat_qualitative %>%
  group_by(ScientificName) %>% 
  summarise(occurence_data_present = n(), .groups="drop") %>%
  st_drop_geometry()
...
print(kbl(list(Rarespec_nat_qualitative_count,Rarespec_nat_quantitative_count))%>%
  kable_classic(full_width = F, html_font = "Cambria"))

write.csv(Rarespec_nat_qualitative_count, file = file.path(Output, paste0(OutputFileName,"_nation_rare_sp_occurence_count.csv")), row.names = FALSE)
...
### 7.3 AQ1: Is the subzone characterised by the presence of many locally rare features?
...
if(!length(RareSpecList$ScientificName) == 0){
  ...
  for (i in 1:NpolID) {
    grid_evaluated <- Qualitative_data[Qualitative_data$grid_id %in% PolIDs$grid_id[i],]
  ...
  produce_map_sf(QPRS,"QPRSCLASS","AQ1")
} else {
  if(!exists("QPRS")) {
  QPRS<-PolIDs
  QPRS$QPRSCLASS<-NA}
...
### 7.4 AQ2: Is the subzone characterised by a high abundance of many locally rare features?
...
### 7.5 AQ3: Is the subzone characterised by the presence of many regionally rare features?
...
if(!length(RareSpecList_region$ScientificName) == 0){
  ...
  # Produce map of the results
  produce_map_sf(QRRS,"QRRSCLASS","AQ3")
} else {
  if(!exists("QRRS")) {
  QRRS<-PolIDs
  QRRS$QRRSCLASS<-NA}
...
### 7.6 AQ4: Is the subzone characterised by high abundance of many regionally rare features?
...
### 7.7 AQ5: Is the subzone characterised by the presence of many nationally rare features?
...
if(!length(RareSpecList_nation$ScientificName) == 0){
  ...
  # Produce map of the results
  produce_map_sf(QNRS,"QNRSCLASS","AQ5")
} else {
  if(!exists("QNRS")) {
  QNRS<-PolIDs
  QNRS$QNRSCLASS<-NA}
...
### 7.8 AQ6: Is the subzone characterised by high abundance of many nationally rare features?
...
### 7.9 AQ7: Is the number of features in the subzone high?

```{r AQ7}
QNFH1 <- data.frame()
features_present <- unique(Qualitative_data$ScientificName)
...
# Produce map of the results
produce_map_sf(QNFH,"QNFHCLASS","AQ7")
...
### 7.10 AQ8: Is the subzone characterised by high counts of many species?
...
### 7.11 AQ 9: Is the abundance of certain species very high in the subzone?
...
### 7.12 AQ 10: Is the presence of ecologically significant features high in the subzone?
...
if(length(ESS) > 0){
  ...
  for(j in ESS){
    info_presence <- Qualitative_data[which(Qualitative_data$ScientificName %in% j), ]
    if(dim(info_presence)[1] == 0){
  ...
  # Produce map of the results
  produce_map_sf(qual_QESS,"qual_QESSCLASS","AQ10")
} else {
  qual_QESS<-PolIDs
...
### 7.13 AQ 11: Is the abundance of ecologically significant features high in the subzone?
...
### 7.14 AQ 12: Is the presence of habitat-forming species (or species formed habitats i.e. biogenic habitats) high in the subzone?
...
if(length(HFKS) > 0){
  ...
  # Produce map of the results
  produce_map_sf(qual_QHFKS,"qual_QHFKSCLASS","AQ12")
} else {
  qual_QHFKS<-PolIDs
...
### 7.15 AQ 13: Is the abundance of habitat-forming species (or species formed habitats i.e. biogenic habitats) high in the subzone?
...
### 7.16 AQ 14: Is the presence of mutualist or symbiotic species high in the subzone?
...
if(length(MSS) > 0){
  ...
  # Produce map of the results
  produce_map_sf(qual_QMSS,"qual_QMSSCLASS","AQ14")
} else {
  qual_QMSS<-PolIDs
...
### 7.17 AQ 15: Is the abundance of symbiotic species high in the subzone?
...
EcoCompQ$Total_unclassified<-SumQ/LenQ
EcoCompQ <-QClassify(EcoCompQ, EcoCompQ$Total_unclassified, "Total")
...
ggplot()+
  geom_sf(data = Result_final$geometry, aes(fill = factor(Result_final$Total)), lwd = 0, colour = "grey90") +
  scale_fill_manual(values = viridisLite::viridis(5), name = "Total", limits = c(1,2,3,4,5), na.value = "grey94", labels = c(
    "1" = "Very Low",
    "2" = "Low",
    "3" = "Medium",
    "4" = "High",
