...
for (i in 1:NpolID) {
  ...
  for (j in features_present) {
    ...
    present <- any(grid_evaluated$ScientificName == j & grid_evaluated$Density > 0, na.rm = TRUE)
...
# Calculation of the average value for each subzone
QNFH2 <- QNFH1 %>%
  mutate(average_value = rowMeans(.[-1], na.rm = TRUE)) %>%  # include na.rm to avoid issues with NA
  rename(grid_id = 1) %>% 
  select(grid_id, average_value)
