...
author: "Willem Boone, Hanneloor Heynderickx"
date: "01/10/2024"
output:
  pdf_document: 
      toc: true
...
**Contact: Willem Boone (willem.boone\@vliz.be)**\
...
*To run only if some packages mentioned above are not
already installed*

```{r install_package}
# Identify packages that are not yet installed
packages_to_install <- my_packages[!(my_packages %in% installed.packages()[, "Package"])]
...
**REMINDER**\
Please ensure that you clean your R-environment before
running the next ecological component.
...
**Output directory**: Filepath for output files; make sure
directory exists.
...
**continuous**: Do you want your results to be displayed on
a continuous scale (from 0 to 5) or on a categorical scale
(5 categories: 1, 2, 3, 4 and 5)

**expert_judgement**, **modelling_data** and
**monitoring_data**: define what type of data has been used.
You must choose one of the following option:
expert_judgement, modelling data or monitoring data. Put
TRUE for the one you used and FALSE for the other.

**component_habitat**: Define if the ecological component
analysed is the benthic habitats. Put TRUE if it is, or
FALSE for any other ecological component.

```{r setting1}
### Path settings for input and output data
InputFile<-"../data/test_data_habitat.csv"
Output<-"../output/"
OutputFileName<-"EVA_EC_habitat"
...
BBT_coordinates = "../data/BBT.geojson"
BBT_layer <- ""
SubzoneShapeName = "../data/grid.geojson"
...
```{r ess}
# no ecological significant species

### Enter your own list here:
ESS <- c("Habitat2", "Habitat7", "Habitat8")
...
```{r mss}
# REMEMBER HABITAT DOESNT APPLY SYMBIOTICs
### Enter your own list here:
MSS <- c("")
```
...
```{r import2}
# Import the file
EcoComp1.0 <- fread(InputFile)

# Remove rows with NA in Density
EcoComp1.0 <- EcoComp1.0[!is.na(Density)]
```
...
**Density** If the data used comes from modelling data, it
is assumed that density represents the percentage of
coverage of a specific feature over a subzone. This
percentage must then be divided by 100 so that the density
ranges from 0 to 1.
...
ggplot() +
  ...
  geom_sf(data = EcoComp1.1_sf, aes(color = ScientificName), shape = 16, size = 2, alpha = 0.8) +
  coord_sf(xlim = c(min(EcoComp1.1$Longitude) - 1, max(EcoComp1.1$Longitude) + 1),
           ylim = c(min(EcoComp1.1$Latitude) - 1, max(EcoComp1.1$Latitude) + 1),
           expand = FALSE) +
  theme_minimal() +
  labs(
    title = "Sampling Points by Scientific Name",
    x = "Longitude",
    y = "Latitude",
    color = "Species"
  ) +
...
Polygon_coordinated_4326 <- st_transform(polygon_bbt, crs = st_crs(4326))
...
```{r bbtcoord_map}
Polygon_coordinated_4326 <- Polygon_coordinated_4326 %>% 
  sf::st_transform(crs="EPSG:4326")
print(ggplot()+
...
grid_polygons_sf <- st_sf(
  grid_id = seq_len(nrow(GRD)),
  geometry = GRD$geometry,
  crs = 4326
)
  
} else if(CreateGrid==TRUE){
  # Create grid 
  GRD <- st_make_grid(Polygon_coordinated_4326$geom, cellsize = GridSize, square = FALSE)
  
  # Create a dataframe with both the grid_polygon and grid_id for each subzone.
  grid_polygons_sf <- st_sf(grid_id = seq_along(GRD[Polygon_coordinated_4326$geom]), 
                          geometry = GRD[Polygon_coordinated_4326$geom], crs = 4326)
}
...
**Function to create the different maps produced in the
script**
...
produce_map <- function(dataset,data_used, title){
  print(ggplot()+
     geom_point(data = dataset,aes(Longitude,Latitude), shape = 1, color="red", size=2) +
...
```{r raresp}
# -------------------------------------------------------------------------
# ORIGINAL CODE (commented out for mockup version)
# -------------------------------------------------------------------------

# # Filter out absences (Density = 0)
# present_data <- subset(assessment_data, Density > 0)
#
# # Aggregate by grid and species to see where species actually occur
# RareSpecList1 <- aggregate(ID ~ grid_id + ScientificName, data = present_data, length)
#
# # Count how many grid cells each species occurs in
# RareSpecList2 <- aggregate(grid_id ~ ScientificName, data = RareSpecList1, length)
#
# # Calculate number of unique grid cells in total
# NpolID <- length(unique(assessment_data$grid_id))
#
# # Species that occur in <= 5% of the grid cells
# RareSpecList <- subset(RareSpecList2, grid_id <= (NpolID * 5 / 100))
#
# # Species that occur in > 5% of the grid cells
# CommonSpecList <- subset(RareSpecList2, grid_id > (NpolID * 5 / 100))

# -------------------------------------------------------------------------
# MOCKUP VERSION (hardcoded lists)
# -------------------------------------------------------------------------

# Hardcoded empty rare species list (no species are considered rare)
RareSpecList <- data.frame(
  ScientificName = character(),  # species names
  grid_id = numeric()            # number of grid cells (count)
)

# Hardcoded common species list (all species considered common in mockup)
CommonSpecList <- data.frame(
  ScientificName = c("Habitat1", "Habitat2", "Habitat3", "Habitat4", "Habitat5", "Habitat6", "Habitat7", "Habitat8"),  # example species
  grid_id = c(64, 64, 64, 64, 64, 64,64, 64)                                   # example grid cell counts
)

# Dataframe required for AQ9 
EcoCompCommon_quant <- Quantitative_data[
  Quantitative_data$ScientificName %in% CommonSpecList$ScientificName, 
]
...
**Write species list and count number of
occurrence/abundance data available**
...
**Distribution map of your common species**

```{r}
# Filter the assessment data for common species
CommonSpec_loc <- assessment_data[assessment_data$ScientificName %in% CommonSpecList$ScientificName, ]

# Write common species locations to a shapefile
st_write(CommonSpec_loc, file.path(Output, "common_species_distribution.shp"), append = FALSE)

# Produce a map for common species presence
produce_map(CommonSpec_loc, "common_sp", "Common species data available")

# Filter the quantitative data for common species
CommonSpec_loc_quantitative <- Quantitative_data[Quantitative_data$ScientificName %in% CommonSpecList$ScientificName, ]

# Write quantitative common species data to a shapefile
st_write(CommonSpec_loc_quantitative, file.path(Output, "common_species_abundance_distribution.shp"), append = FALSE)

# Produce a map for common species abundance
produce_map(CommonSpec_loc_quantitative, "common_sp_abundance", "Common species abundance data available")
...
```{r}
# Summarise occurrence data for common species
CommonSpec_loc_count <- CommonSpec_loc %>%
  group_by(ScientificName) %>% 
  summarise(occurence_data_present = n(), .groups = "drop") %>%
  st_drop_geometry()

# Summarise abundance/quantitative data for common species
CommonSpec_loc_quantitative_count <- CommonSpec_loc_quantitative %>%
  group_by(ScientificName) %>% 
  summarise(abundance_data_present = n(), .groups = "drop") %>%
  st_drop_geometry()
...
print(
  kbl(
    list(CommonSpec_loc_count, CommonSpec_loc_quantitative_count)
  ) %>% 
    kable_classic(full_width = F, html_font = "Cambria")
)

# Write CSV files for common species
write.csv(
  CommonSpec_loc_count,
  file = file.path(Output, paste0(OutputFileName, "_common_sp_count.csv")),
  row.names = FALSE
)

write.csv(
  CommonSpec_loc_quantitative_count,
  file = file.path(Output, paste0(OutputFileName, "_common_sp_abundance_count.csv")),
  row.names = FALSE
)
...
# GRD_region <- st_make_grid(Polygon_coordinated_4326$geom, cellsize = 50000, square = TRUE)
#   
# # Create a dataframe with the grid_polygon and grid_id for each subzone
# grid_polygons_sf_region <- st_sf(
#   grid_id = seq_along(GRD_region[Polygon_coordinated_4326$geom]), 
#   geometry = GRD_region[Polygon_coordinated_4326$geom], 
#   crs = 4326
# )
#
...
#
# # Create unique location numbers
# EcoComp_region$LocationNumber <- (round(EcoComp_region$Latitude, digits = 4)) / 100 + 
#   10000 * (round(EcoComp_region$Longitude, digits = 4))
#
...
#
# # Listing those species that occur in less than 2% of the gridcells
# RareSpecList_region <- RareSpecList2_region[
#   which(RareSpecList2_region$grid_id <= (NpolID * 2 / 100)), ]

# -------------------------------------------------------------------------
# MOCKUP VERSION (hardcoded empty list)
# -------------------------------------------------------------------------

RareSpecList_region <- data.frame(
  ScientificName = character(),  # species names
  grid_id = numeric()            # number of grid cells (count)
)
...
# GRD_nation <- st_make_grid(Polygon_coordinated_4326$geom, cellsize = 10000, square = TRUE)
#   
# # Create a dataframe with grid_polygons and grid_ids for each subzone
# grid_polygons_sf_nation <- st_sf(
#   grid_id = seq_along(GRD_nation[Polygon_coordinated_4326$geom]), 
#   geometry = GRD_nation[Polygon_coordinated_4326$geom], 
#   crs = 4326
# )
#
# # Get Spatialpoint from dataset
...
#
# # Create unique location numbers
# EcoComp_nation$LocationNumber <- (round(EcoComp_nation$Latitude, digits = 4)) / 100 + 
#   10000 * (round(EcoComp_nation$Longitude, digits = 4))
#
# # Creating a list of unique grid_id - ScientificName combinations
...
#
# # Listing those species that occur in less than 0.5% of the gridcells
# RareSpecList_nation <- RareSpecList2_nation[
#   which(RareSpecList2_nation$grid_id <= (NpolID * 0.5 / 100)), ]

# -------------------------------------------------------------------------
# MOCKUP VERSION (hardcoded empty list for nationally rare species)
# -------------------------------------------------------------------------

RareSpecList_nation <- data.frame(
  ScientificName = character(),  # species names
  grid_id = numeric()            # number of grid cells (count)
)
...
```{r AQ7}
QNFH1 <- data.frame()
features_present <- unique(as.character(assessment_data$ScientificName))  # ensure character
...
for (i in 1:NpolID) {
  ...
  for (j in features_present) {
    # Extract densities for this species in the current grid
    densities <- grid_evaluated$Density[grid_evaluated$ScientificName == j]
    
    # Determine the presence value
    if (length(densities) == 0) {
      # species not recorded at all in this grid
      value <- 0
    } else if (all(is.na(densities))) {
      # all density values are NA → output NA
      value <- NA
    } else if (any(densities > 0, na.rm = TRUE)) {
      # at least one density > 0 → presence
      value <- 5
    } else {
      # all densities are 0 (or ignored NA) → absence
      value <- 0
    }
    
    grid_vector <- append(grid_vector, value)
  }
...
# Calculation of the average value for each subzone
QNFH2 <- QNFH1 %>%
  mutate(average_value = rowMeans(.[-1], na.rm = FALSE)) %>%  # keep NA if any
  rename(grid_id = 1) %>% 
  select(grid_id, average_value)
...
# If QNFH does not exist, create empty with NAs
if (!exists("QNFH")) {
  QNFH <- PolIDs
  QNFH$QNFHCLASS <- NA
}
...
# Filter out species not present in the data
for(i in CommonSpecList$ScientificName){
    info_presence <- Quantitative_data[Quantitative_data$ScientificName == i, ]
    if(nrow(info_presence) == 0){
...
if(length(CommonSpecList_quant) > 0){
  for(j in CommonSpecList_quant){
    # Get data for that species
    lrf_abundance <- Quantitative_data[Quantitative_data$ScientificName == j, ] %>% st_drop_geometry()
    
    if(nrow(lrf_abundance) > 2){
      # Average abundance per grid
      df_lrf_abundance_average <- lrf_abundance %>%
        dplyr::select(grid_id, ScientificName, Density) %>%
        group_by(grid_id) %>%
        summarise(average = mean(Density, na.rm = TRUE), .groups = "drop")
      
      # Compute lrf_value, preserving NA
      df_lrf_value <- df_lrf_abundance_average %>%
        inner_join(unique(lrf_abundance[, c("grid_id","ScientificName")]), by = "grid_id") %>%
        mutate(lrf_value = ifelse(all(is.na(average)), NA, 5 * (average / max(average, na.rm = TRUE))))
      
    } else next
    
    # Join to final dataframe
    QHCMSa <- left_join(QHCMSa, df_lrf_value[, c("grid_id", "lrf_value")], by = "grid_id")
  }
  
  # Average across species per grid, preserving NA if all are NA
  QHCMSb <- QHCMSa %>%
    rowwise() %>%
    mutate(average_lrf_value = if(all(is.na(c_across(-grid_id)))) NA_real_ else mean(c_across(-grid_id), na.rm = TRUE)) %>%
    ungroup() %>%
...
if(length(ESS) > 0){
  ...
  for(j in ESS){
    info_presence <- assessment_data[assessment_data$ScientificName %in% j, ]
    if(nrow(info_presence) == 0){
  ...
  for (i in 1:NpolID) {
    grid_evaluated <- assessment_data[assessment_data$grid_id == PolIDs$grid_id[i], ]
    grid_vector <- PolIDs$grid_id[i]
    
    for(k in ESS_list){
      # Extract densities for this species in the grid
      densities <- grid_evaluated$Density[grid_evaluated$ScientificName == k]
      
      # Determine presence value
      if(all(is.na(densities))){
        grid_vector <- append(grid_vector, NA)     # all NA → NA
      } else if(any(densities > 0, na.rm = TRUE)){
        grid_vector <- append(grid_vector, 5)      # any positive → presence
      } else {
        grid_vector <- append(grid_vector, 0)      # all 0 → absence
      }
    }
  ...
  qual_QESS2 <- qual_QESS1 %>%
    mutate(average_value = rowMeans(.[-1], na.rm = TRUE)) %>%  # na.rm TRUE for rowMeans
    rename(grid_id = 1) %>% 
    select(grid_id, average_value)
...
if(length(HFKS) > 0){
  ...
  HFKS_list <- HFKS[sapply(HFKS, function(sp) any(assessment_data$ScientificName == sp))]
  
  # Initialize result data frame
  qual_QHFKS1 <- data.frame(grid_id = PolIDs$grid_id)
  
  # Loop through species and calculate presence per grid
  for(k in HFKS_list){
    presence_vec <- sapply(PolIDs$grid_id, function(gid){
      densities <- assessment_data$Density[assessment_data$grid_id == gid & assessment_data$ScientificName == k]
      if(all(is.na(densities))){
        return(NA)          # all NA → NA
      } else if(any(densities > 0, na.rm = TRUE)){
        return(5)           # any positive → presence
      } else {
        return(0)           # all 0 → absence
      }
    })
    qual_QHFKS1[[k]] <- presence_vec
  }
  
  # Compute average per grid
  qual_QHFKS2 <- qual_QHFKS1 %>%
    mutate(average_value = rowMeans(.[-1], na.rm = TRUE)) %>%  # ignore NA in averages
    select(grid_id, average_value)
...
This score is calculated based on the equation present in
the guideline. It needs 3 parameters: AQi, N and wi.

**AQi**: identifies the ith Assessment Question, which in
the equation is assigned a value of 1 if it has been
answered or 0 if not answered.

**N**: total number of questions defined for each EC (e.g.,
N=15 for macrobenthos, N=13 for marine benthic habitats)

**wi**: weight assigned to AQ, the value is between 1 and 5
and depends on the type of data (modelling/monitoring data
or expert judgment) and the data availability.
...
AQi_results <-EcoCompQ_base %>%
   mutate(across(.cols = everything(), ~ ifelse(is.na(.x), 0, 1)))

# Get wi for each subzones 
# Count number of samples for monitoring data
DAV <- assessment_data %>%
  group_by(grid_id) %>%
...
if(modelling_data == TRUE){
  DAV <- assessment_data %>%
    group_by(grid_id) %>%
    summarise(area_covered = sum(Density, na.rm = TRUE)) %>% 
    st_drop_geometry()
}


# If the number of subzones covered by the data is less than the number of subzones present in the bbt, than the minimum of the range will always be 0
...
# change nb_sample into categories (low, Medium-Low, Medium, Medium-High, High), depending on the data used 
if (expert_judgement == TRUE){
  DAV2 <- DAV
}else if(modelling_data == TRUE){
  DAV2 <- DAV %>%
    mutate(categories_dav = ifelse(area_covered >= 0 & area_covered <= 0.2, "Low",
                        ifelse(area_covered > 0.2 & area_covered <= 0.4, "Medium-Low",
                               ifelse(area_covered > 0.4 & area_covered <= 0.6, "Medium",
                                      ifelse(area_covered > 0.6 & area_covered <= 0.8, "Medium-High",
                                             ifelse(area_covered > 0.8 & area_covered <= 1, "High", NA)))))) %>%
  select(grid_id,categories_dav)
} else if(monitoring_data == TRUE){
  DAV2 <- DAV %>%
    mutate(categories_dav = ifelse(nb_sample >= min_data & nb_sample <= (min_data+X_data), "Low",
                        ifelse(nb_sample > (min_data+X_data) & nb_sample <= (min_data+(2*X_data)), "Medium-Low",
                               ifelse(nb_sample > (min_data+(2*X_data)) & nb_sample <= (min_data+(3*X_data)), "Medium",
                                      ifelse(nb_sample > (min_data+(3*X_data)) & nb_sample <= (min_data+(4*X_data)), "Medium-High",
                                             ifelse(nb_sample > (min_data+(4*X_data)) & nb_sample <= max_data, "High", NA)))))) %>%
  select(grid_id,categories_dav)
}

# Mutate those categories into wi, based on the type of data used
...
if (expert_judgement == TRUE){
  ...
  wi_results <- DAV2 %>%
    mutate(wi = ifelse(categories_dav == "Low", 1,
                        ifelse(categories_dav == "Medium-Low", 1,
                               ifelse(categories_dav == "Medium", 2,
                                      ifelse(categories_dav == "Medium-High", 3,
                                             ifelse(categories_dav == "High", 3, NA)))))) %>%
    select(grid_id,wi)
} else if(monitoring_data == TRUE){
  wi_results <- DAV2 %>%
    mutate(wi = ifelse(categories_dav == "Low", 1,
                        ifelse(categories_dav == "Medium-Low", 2,
                               ifelse(categories_dav == "Medium", 3,
                                      ifelse(categories_dav == "Medium-High", 4,
                                             ifelse(categories_dav == "High", 5, NA)))))) %>%
    select(grid_id,wi)
}
...
EcoComp_DAV2$conf_score <- rowSums(EcoComp_DAV2[ ,3:length(colnames(EcoComp_DAV2))], na.rm=TRUE)/N
...
LenQ <- apply(EcoCompQ, 1, function(t) sum(!is.na(t)))     # count of non-NA values
SumQ <- apply(EcoCompQ, 1, sum, na.rm = TRUE)              # sum of non-NA values
MaxQ <- apply(EcoCompQ, 1, max, na.rm = TRUE)              # NEW: maximum of non-NA values

# Add computed columns
EcoCompQ$SumQ <- SumQ
EcoCompQ$LenQ <- LenQ
EcoCompQ$Total <- MaxQ                                     # Use max instead of mean

# Reset nb_sample when modelling_data = TRUE
if (modelling_data == TRUE) {
  DAV$nb_sample <- 0
}
...
# Add grid ID and confidence score
EcoCompQ <- data.frame(cbind(PolIDs, EcoCompQ, EcoComp_DAV2$conf_score))

# Combine with grid geometry
...
write.csv(Result_final, 'result_habitats.csv', row.names = TRUE)
```
